...
impl AiClient {
    ...
    fn find_tool_name_for_message(messages: &[Message], tool_call_id: &str) -> Option<String> {
    ...
    fn map_messages_to_ollama(&self, messages: &[Message]) -> Vec<OllamaMessage> {
        messages
            .iter()
            .map(|message| {
                let tool_name = if message.role == Role::Tool {
                    message
                        .tool_call_id
                        .as_ref()
                        .and_then(|id| Self::find_tool_name_for_message(messages, id))
                } else {
                    None
                };
    ...
    fn map_ollama_tool_calls(tool_calls: &[OllamaToolCall]) -> Vec<ToolCall> {
        tool_calls
            .iter()
            .enumerate()
            .map(|(index, call)| ToolCall {
                id: call
                    .id
                    .clone()
                    .unwrap_or_else(|| format!("ollama_tool_call_{}", index)),
                tool_type: call.tool_type.clone(),
    ...
    pub async fn chat_with_tools(
        ...
        if self.provider == AiProvider::Ollama {
            let url = format!("{}/api/chat", self.get_base_url());
            let ollama_messages = self.map_messages_to_ollama(&messages);
            ...
            let mut body = body;
            let has_tools = if let Some(ref provided_tools) = tools {
                if !provided_tools.is_empty() {
                    body["tools"] = json!(provided_tools);
                    true
                } else {
                    false
                }
            ...
            if !resp.status().is_success() && has_tools {
                let status = resp.status();
                let text = resp.text().await.unwrap_or_default();
                let lower = text.to_lowercase();
                if lower.contains("tool") || lower.contains("function") || lower.contains("support") {
                    return Err(AiError::ToolIncompatibility(format!(
                        "Ollama model '{}' does not appear to support tool calling: {}",
                        self.model, text
                    )));
                }
                return Err(AiError::Stream(format!(
            ...
            let tool_calls = ollama_resp
                ...
                .tool_calls
                .as_ref()
                .map(|calls| Self::map_ollama_tool_calls(calls));
